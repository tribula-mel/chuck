...
def chuck_platform_collision (game):
   player = game.get_player_context ()
...
def do_chuck_jump_state (game):
   ...
   if ((player.get_sandbox (tile_x, tile_y)) & 0x1) != 0:
      ...
      if player.get_sandbox (tile_x, tile_y + 1) != 1:
         if gfx[1] < 0xae:
            jump_934d (game, dx, dy)
            return
   dy = -1
